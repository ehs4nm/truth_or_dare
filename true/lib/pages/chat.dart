...
class _ChatPageState extends State<ChatPage> {
    ...
    if (result != null && result.files.single.bytes != null) {
      ...
      try {
        ...
        final reference = await Supabase.instance.client.storage
            .from(buket)
            .uploadBinary('${widget.room.id}/${const Uuid().v1()}-$name', bytes!, fileOptions: FileOptions(contentType: mimeType));
  ...
  Future<void> _handlePreviewDataFetched(
    ...
    await SupabaseChatCore.instance.updateMessage(updatedMessage, widget.room.id);
  }
          ...
          builder: (context, snapshot) => Chat(
            ...
            onMessageVisibilityChanged: (message, visible) async {
              if (message.status != types.Status.seen && message.author.id != SupabaseChatCore.instance.supabaseUser!.id) {
                await SupabaseChatCore.instance.updateMessage(message.copyWith(status: types.Status.seen), widget.room.id);
              }
            },
