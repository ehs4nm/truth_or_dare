...
///
// void main() {
//   runApp(MaterialApp(home: TapselApp()));
//   TapsellPlus.instance.initialize(TapsellConstant.app_id);
// }

class TapselApp extends StatefulWidget {
  @override
  _TapselAppState createState() => _TapselAppState();
}

class _TapselAppState extends State<TapselApp> {
  ...
  void log(String message, {String? tag}) => setState(() {
        _list.add({'tag': tag ?? 'Command', 'message': message, 'date': DateTime.now().toIso8601String()});
      });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      key: _scaffoldKey,
      appBar: AppBar(backgroundColor: Colors.blueGrey, title: const Text('TapsellPlus - 2.2.4')),
      body: Center(
          child: Column(
          ...
          Expanded(
            ...
            child: PageView(
              children: networks.values
                  .map((e) => Center(child: Text("←  $e  →", style: TextStyle(color: Colors.blueGrey, fontWeight: FontWeight.bold, fontSize: 16))))
                  .toList(),
                ...
                children: getActions().map((e) {
                  return ListTile(
                    title: Text('${e['ready'] ? '🆗' : '⛔'} ${e['name']}',
                        style: TextStyle(color: Colors.white.withOpacity(e['ready'] ? 1.0 : 0.5), fontWeight: FontWeight.bold)),
                    subtitle: Text(e['desc'], style: TextStyle(color: Colors.white60)),
                    onTap: () {
                      if (e['ready'])
                        e['onClick']?.call();
                      else
                ...
                children: [
                  Container(
                    ...
                    child: ListView(
                      ...
                      children: _list
                          .map((e) {
                            final smallTextStyle = TextStyle(fontSize: 14, color: Colors.white);
                            return ListTile(
                              title: RichText(
                                text: TextSpan(children: [
                                  ...
                                  TextSpan(text: e['message'].toString(), style: smallTextStyle),
                                ]),
                              ),
                              subtitle: Text(e['date'].toString(), style: smallTextStyle.copyWith(fontSize: 12, color: Colors.white60)),
                            );
                          })
                          .toList()
                          .reversed
                  ...
                  Positioned(child: IconButton(icon: Icon(Icons.clear, color: Colors.red), onPressed: clearLogs), right: 1.0, top: 1.0)
                ],
              )),
          isAdmobNativeLoaded
              ? Container(
  ...
  List<Map<String, dynamic>> getActions() => [
        ...
        },
        {
          ...
          'onClick': () {
            ...
            TapsellPlus.instance.requestStandardBannerAd(zoneId, TapsellPlusBannerType.BANNER_320x50, onResponse: (map) {
        ...
        },
        {
          ...
          'onClick': () {
            final id = responseId;
            if (id.isNotEmpty) {
              TapsellPlus.instance.showStandardBannerAd(id, TapsellPlusHorizontalGravity.TOP, TapsellPlusVerticalGravity.CENTER,
        ...
        },
        {
          ...
          'onClick': () {
            ...
            TapsellPlus.instance.requestNativeAd(zoneId, onResponse: (Map<String, String> response) {
              setResponseId(response['response_id'] ?? "");
              setUnitId(response['adnetwork_zone_id'] ?? "");
            }, onError: (Map<String, String> error) {
              log('Error requesting ad - ${error['message']}', tag: '--native-status');
            }).catchError((error) {
        ...
        },
        {
          ...
          'onClick': () {
            final id = responseId;
            if (id.isNotEmpty) {
              TapsellPlus.instance.showNativeAd(id, admobFactoryId: factoryId, onOpened: (nativeAd) {
...
class TapsellConstant {
  static const app_id = 'oerloelebgmlrepofrtojjqfgmrcnqmqetoggimjbttjoqdoeenjjkcsobijnldkolrdhn';
  static const rewarded_video_zone = '6627717649bdb742ad13659d';
  static const interstitial_zone = '6627712c8c400e6dbb15886d';
  static const native_banner_zone = '662771bb6fd93604cf618f91';
  static const standard_zone = '662771d86fd93604cf618f92';

  static const zoneIds = {
    "Tapsell": {
      "REWARDED": '6627717649bdb742ad13659d',
      "INTERSTITIAL": '6627712c8c400e6dbb15886d',
      "NATIVE": '662771bb6fd93604cf618f91',
      "STANDARD": '662771d86fd93604cf618f92',
    },
    "Google AdMob": {
    ...
    "Chart boost": {"REWARDED": '5cfaa8cee8d17f0001ffb290', "INTERSTITIAL": '60c5b303d756bf0001891f1c'},
    "Unity Ads": {
    ...
    return Material(
      ...
      child: Column(
        ...
        children: [
          ...
          Expanded(flex: 7, child: Image.network(portraitImageUrl)),
          SizedBox(
            height: 15,
          ),
          Expanded(
